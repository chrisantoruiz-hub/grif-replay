...
static FILE *dssd_efront_c_file = NULL;
void open_dssd_efront_c_file(void){
   dssd_efront_c_file = fopen("dssd_efront_coinc.dat", "w");
   if( dssd_efront_c_file == NULL ){ perror("fopen dssd_efront_coinc.dat"); }
}
void close_dssd_efront_c_file(void){
...
static FILE *ic_sum_vs_dssd_efront_file = NULL;
void open_ic_sum_vs_dssd_efront_file(void){
   ic_sum_vs_dssd_efront_file = fopen("ic_sum_vs_dssd_efront_singles.dat", "w");
   if( ic_sum_vs_dssd_efront_file == NULL ){ perror("fopen ic_sum_vs_dssd_efront_singles.dat"); }
}
void close_ic_sum_vs_dssd_efront_file(void){
   if( ic_sum_vs_dssd_efront_file != NULL ){ fclose(ic_sum_vs_dssd_efront_file); ic_sum_vs_dssd_efront_file = NULL; }
}

static FILE *ic_sum_vs_dssd_efront_c_file = NULL;
void open_ic_sum_vs_dssd_efront_c_file(void){
   ic_sum_vs_dssd_efront_c_file = fopen("ic_sum_vs_dssd_efront_coinc.dat", "w");
   if( ic_sum_vs_dssd_efront_c_file == NULL ){ perror("fopen ic_sum_vs_dssd_efront_coinc.dat"); }
}
void close_ic_sum_vs_dssd_efront_c_file(void){
   if( ic_sum_vs_dssd_efront_c_file != NULL ){ fclose(ic_sum_vs_dssd_efront_c_file); ic_sum_vs_dssd_efront_c_file = NULL; }
}
...
int init_default_histos(Config *cfg, Sort_status *arg)
{
   ...
   open_ic_sum_file();
   open_ic_sum_c_file();
   open_ic_sum_vs_dssd_efront_file();
   open_ic_sum_vs_dssd_efront_c_file();
...
//         // bgo_e0_e1: fill highest two BGO energies — needs a small find-top-two loop
      ...
      for(i=0; i<SB_MAXCHAN; i++){
       ...
       // DSSD
         ...
         if( dssd_front_e > 0 ){
            ic_sum_vs_dssd_efront->Fill(ic_sum_vs_dssd_efront, dssd_front_e, (int)sum, 1);
            if( ic_sum_vs_dssd_efront_file != NULL ) fprintf(ic_sum_vs_dssd_efront_file, "%d %.1f %.1f %ld\n", sort_event_count, sum, dssd_front_e, ptr->ts);
         }
	 ...
	 tmp = ptr; ptr = alt; alt = tmp;
      ...
      // }
       ...
       // DSSD (from tail)
        ...
        tail->ic_sum = sum;
        if( dssd_front_e > 0 ){
            ic_sum_vs_dssd_efront_c->Fill(ic_sum_vs_dssd_efront_c, dssd_front_e, (int)sum, 1);
            if( ic_sum_vs_dssd_efront_c_file != NULL ) fprintf(ic_sum_vs_dssd_efront_c_file, "%d %.1f %.1f %ld\n", sort_event_count, sum, dssd_front_e, alt->ts);
        }
        if( tail->ic_energy[0] > 0 && tail->ic_energy[1] > 0 )
